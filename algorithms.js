...
.directive( "questionsAlgoritms", function() {
	return {
		...
		template: "<a name='{{name}}'/>" +
				  ...
				  "<ul>" +
				  "	<li ng-repeat='question in random'><span class='w3-tooltip' name='algorithms' id='{{question.id}}'>Rank {{question.rank}} <span class='w3-text w3-tag w3-khaki w3-round w3-small' ng-click='rank(question.id);'>Click to Suggest Another Ranking</span></span> {{question.question}}<br/><br/>" +
				  "	<span ng-show='answers'><span class='answer'>{{question.answer}}</span> <button class='w3-btn w3-khaki w3-round w3-small' ng-click='better(question.id);'>Suggest A Better Answer</button><br/></span>Correct <input class='{{name}}-correct' type='checkbox'/><br/><br/></li>" +
				  "</ul>" +
